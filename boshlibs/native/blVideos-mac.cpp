//--------------------------------------------------------------------------
//  Riproduzione di un .mp4 dentro una Image di mojo2, su macOS (Desktop).
//
//  Usa AVFoundation (AVAssetReader + AVAssetReaderTrackOutput), che fa gia'
//  parte del sistema: nessuna libreria esterna da distribuire. L'output e'
//  configurato per consegnare fotogrammi kCVPixelFormatType_32BGRA, quindi
//  la conversione dal formato del filmato la fa il sistema.
//
//  Struttura identica alla versione Windows (Media Foundation): un thread di
//  sfondo decodifica e tiene pronto UN fotogramma gia' convertito in RGBA, il
//  thread che disegna si limita a una memcpy dentro il DataBuffer (_CopyFrame).
//  I due buffer _rgba/_work si scambiano con uno swap di puntatori, quindi
//  dopo il primo fotogramma non si alloca piu' niente.
//
//  AVAssetReader NON si riavvolge: una volta arrivato in fondo (o in errore)
//  va ricreato daccapo per ripartire. E' per questo che _OpenReader() e'
//  richiamato sia all'apertura sia a ogni riavvolgimento per il loop, mentre
//  la dimensione del filmato (serve al chiamante per creare la Image) viene
//  letta una volta sola in _Open, con una interrogazione separata e piu'
//  leggera (solo la traccia, nessun reader).
//
//  Non viene applicato AVAssetTrack.preferredTransform: AVAssetReader
//  restituisce i pixel COSI' COME SONO in memoria, non ruotati secondo quella
//  matrice (la applica solo chi compone, es. AVPlayer). Usare la dimensione
//  "esposta" (ruotata) come larghezza/altezza dichiarate, tenendo invece i
//  pixel nella loro orientazione originale, romperebbe la Image (dimensioni
//  dichiarate diverse da quelle del buffer vero). Un filmato girato in
//  verticale da telefono esce quindi ruotato, esattamente come su Windows,
//  che ha la stessa limitazione.
//
//  Il progetto Xcode di questo target (targets/glfw3/template/xcode) NON ha
//  un meccanismo per aggiungere framework da un modulo (a differenza del
//  target iOS, che legge #LIBS+="X.framework"): l'elenco frameworks del
//  progetto e' fisso. CoreVideo.framework c'e' gia'; AVFoundation.framework e
//  CoreMedia.framework vanno aggiunti a mano al progetto (una volta sola,
//  vedi PORT-MAC-LINUX.md) perche' AVAssetReader/CMSampleBuffer possano
//  linkare.
//
//  ATTENZIONE ARC: questo target NON compila con Automatic Reference
//  Counting (CLANG_ENABLE_OBJC_ARC non e' impostato nel progetto, quindi e'
//  spento). Gli oggetti Objective-C creati con alloc/init vanno rilasciati a
//  mano. Le chiamate che restituiscono oggetti autoreleased (es.
//  tracksWithMediaType:, URLAssetWithURL:options:) sono racchiuse in
//  @autoreleasepool: il thread di decodifica e' un std::thread nudo, senza
//  il pool implicito che avrebbe un NSThread/la run loop principale, quindi
//  senza quel blocco andrebbero perse silenziosamente ("leak", non crash,
//  ma comunque da evitare) a ogni apertura/riavvolgimento.
//
//  AUDIO: un secondo AVAssetReader indipendente, aperto sullo stesso file,
//  con l'output impostato sulla sola traccia audio e convertito a PCM 16 bit
//  stereo tramite outputSettings (stessa idea del video: si chiede il
//  formato che serve e la conversione la fa AVFoundation da sola, non serve
//  swresample/AVAudioConverter a mano). Un secondo thread di sfondo tiene in
//  coda 4 buffer OpenAL e li riempie via via che vengono consumati - stesso
//  schema "coda di buffer" di blVideos-win.cpp/blVideos-linux.cpp: l'audio
//  si scandisce da solo a tempo reale, non serve un orologio manuale come
//  per il video. Il contesto OpenAL e' quello che mojo.audio apre gia'
//  all'avvio (stesso backend OpenAL del target glfw3 su Windows/Linux): e'
//  globale fra i thread in OpenAL, quindi le chiamate AL da questo thread
//  sono sicure senza doverne aprire uno nostro. OpenAL.framework e' gia' nel
//  progetto Xcode di questo target (lo usa mojo.audio), non serve aggiungere
//  niente.
//
//  A differenza di Media Foundation, AVAssetReader non ha uno stato
//  "campione non ancora pronto senza essere a fine flusso": copyNextSample-
//  Buffer che torna nil vuol dire sempre o fine flusso o errore, mai
//  "riprova fra un attimo". Semplifica la logica di _FillAudioBuffer rispetto
...
private:
    ...
    /*  Due buffer che si scambiano: _work e' quello che il decoder riempie,
        _rgba quello pronto per chi disegna (stesso schema della versione
        Windows, vedi li' per il perche' - evitare di allocare a ogni
        fotogramma). */
...
bool BBblVideo::_Open( String path ){
    ...
    /*  L'audio e' un extra: se il file non ne ha, o la conversione a PCM
        fallisce, si continua senza. Non e' _failed, il video sopra funziona
        comunque. */
    ...
    if( src && w>0 && h>0 ){
        ...
        for( int y=0;y<h;++y ){
            ...
            for( int x=0;x<w;++x ){
                ...
                /*  kCVPixelFormatType_32BGRA in memoria e' B,G,R,A: letta
                    come parola little endian vale B | G<<8 | R<<16 | A<<24,
                    stesso schema del caso Windows (RGB32). mojo2 vuole
                    R,G,B,A: bastano tre spostamenti. Alfa forzato a pieno
                    come su Windows - un fotogramma video e' sempre opaco. */
...
void BBblVideo::_Decode(){
    ...
    while( _running ){
        ...
        if( !sample ){
            ...
            /* fine flusso (o errore): AVAssetReader non si riavvolge, va
               ricreato daccapo per ripartire */
            @autoreleasepool{
                [output release];
                [reader release];
            }
        ...
        /* i tempi di CMTime sono un valore/scala, non secondi diretti */
        ...
        /*  Se siamo rimasti molto indietro si RIAGGANCIA l'orologio invece
            di scartare il fotogramma (stesso motivo della versione Windows:
            senza, un ritardo temporaneo del decoder blocca il filmato per
            sempre, perche' ogni fotogramma successivo risulta "in ritardo"
            e viene buttato). */
        if( late>0.25 ){
