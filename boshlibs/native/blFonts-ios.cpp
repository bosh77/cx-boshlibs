//--------------------------------------------------------------------------
//  blFonts - nativo iOS. Porta blFonts-mac.cpp: stesso CoreText/CoreGraphics
//  (identici sulle due piattaforme), le uniche differenze sono i due punti
//  dove macOS usa AppKit:
//
//   - _GetFontListString usava NSFontManager (AppKit, non c'e' su iOS):
//     su iOS l'elenco delle famiglie installate si legge da
//     [UIFont familyNames], che e' UIKit.
//   - ApplicationServices.framework non esiste su iOS: l'#include si toglie,
//     CoreText.h e CoreGraphics.h bastano da soli (e' cosi' anche su macOS,
//     quell'umbrella li' non aggiungeva niente che servisse qui).
//
//  Un fix trovato in questo file va portato a mano anche in blFonts-mac.cpp
//  (o viceversa): stesso motivo di blVideos-mac.cpp/blVideos-ios.cpp, "un
//  file per piattaforma" invece di uno importato da entrambe.
//
//  MAI COMPILATO PRIMA su un device vero: provato solo su iOS Simulator
//  (vedi PORT-MAC-LINUX.md). Le differenze fra simulatore e device per
//  CoreText sono storicamente minime, ma non e' stato verificato qui.
//--------------------------------------------------------------------------
...
int BBblFont::_SelectFont(){
    ...
    /* niente pannello di selezione font su iOS: come su macOS, e' un punto
       dell'API che resta un semplice stub. */
    return 0;
}
...
/*  Font caricato da un FILE dell'applicazione invece che da una famiglia
    installata. Come su macOS: si crea un CGFont dal file e lo si passa a
    CoreText, senza registrare niente nel sistema.
